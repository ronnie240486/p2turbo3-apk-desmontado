...
public final class ItemCatDao_Impl implements ItemCatDao {
    ...
    public ItemCatDao_Impl(p pVar) {
        this.__db = pVar;
        this.__insertionAdapterOfItemCat = new e(pVar) { // from class: com.legacy.prime.BancoSql.livetv.ItemCatDao_Impl.1
        ...
        this.__preparedStmtOfLimpar = new u(pVar) { // from class: com.legacy.prime.BancoSql.livetv.ItemCatDao_Impl.2
            @Override // androidx.room.u
            public String createQuery() {
    ...
    public String getAdultCategoryId() {
        s sVarO = s.o(0, "SELECT id FROM cat WHERE LOWER(name) LIKE '%adul%'    OR LOWER(name) LIKE '%+18%' LIMIT 1");
        this.__db.assertNotSuspendingTransaction();
        Cursor cursorG = p061k4.a.G(this.__db, sVarO);
        try {
            String string = null;
